  ...
  return (
    <section ref={containerRef} id="hero" className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
      ...
      <motion.div style={{ y: bgY }} className="absolute inset-0 scale-110">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-ae-dark/75 via-ae-dark/40 to-ae-dark/95" />
        <div className="absolute inset-0 bg-gradient-to-r from-ae-dark/60 via-transparent to-ae-dark/30" />
      </motion.div>
      ...
      <motion.div style={{ y: textY, opacity }} className="relative z-10 text-center px-6 max-w-5xl mx-auto pt-24">
        ...
        <motion.div custom={0} variants={FADE_UP} initial="hidden" animate="visible" className="inline-flex items-center gap-2 mb-8">
          ...
          <span className="text-xs tracking-[0.3em] uppercase text-ae-gold font-medium">{t.label}</span>
          <span className="w-8 h-px bg-ae-gold/60" />
        </motion.div>
        ...
        <motion.p custom={2} variants={FADE_UP} initial="hidden" animate="visible"
          className="font-serif text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-[1.05] italic text-ae-gold mb-8">
          {t.titleLine2}
        </motion.p>

        {/* Description */}
        <motion.p custom={3} variants={FADE_UP} initial="hidden" animate="visible"
          className="text-ae-sand/70 text-lg md:text-xl leading-relaxed max-w-3xl mx-auto mb-10">
          {t.description}
        </motion.p>
        ...
        <motion.div custom={4} variants={FADE_UP} initial="hidden" animate="visible"
          ...
          <a href="#what-is"
            className="px-7 py-3.5 rounded-xl text-sm font-semibold bg-ae-gold hover:bg-ae-gold-light text-ae-dark transition-colors shadow-lg shadow-ae-gold/20">
            {t.btn1}
          </a>
          <a href="#projects"
            className="px-7 py-3.5 rounded-xl text-sm font-medium bg-white/[0.07] hover:bg-white/[0.12] border border-white/[0.12] text-ae-sand/80 hover:text-ae-sand backdrop-blur-sm transition-colors">
            {t.btn2}
          </a>
        </motion.div>
        ...
        <motion.div custom={5} variants={FADE_UP} initial="hidden" animate="visible"
          ...
          {stats.map((stat, i) => (
            <div key={i} className="bg-ae-dark/60 px-6 py-5 text-center">
              <div className="font-serif text-2xl md:text-3xl font-bold text-ae-gold mb-1">{stat.value}</div>
              <div className="text-ae-sand/50 text-xs leading-snug">{stat.label}</div>
            </div>
          ))}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2 }} className="mt-12 flex flex-col items-center gap-2">
          <span className="text-ae-sand/30 text-xs tracking-widest uppercase">
