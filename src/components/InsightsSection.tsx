...
export default function InsightsSection() {
  ...
  // Show the 3 most recent posts, sorted by date descending
  ...
  return (
    <section id="insights" className="bg-ae-stone py-24 md:py-32">
      <div ref={ref} className="max-w-6xl mx-auto px-6">
        ...
        {/* Articles grid — real posts from posts.ts */}
        <div className="grid md:grid-cols-3 gap-6">
          {recentPosts.map((post, i) => {
            const postLang = post.lang[lang] ?? post.lang["en"];
            const href = `/${lang}/blog/${post.slug}`;
            const dateDisplay = formatDate(post.date, lang);
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <span className="px-2.5 py-1 rounded-md bg-ae-teal/10 border border-ae-teal/25 text-ae-teal text-[10px] font-bold tracking-wide uppercase">
                      {lang === "ar" ? "مقال" : "Insight"}
                    </span>
                    <span className="text-ae-muted text-xs">{dateDisplay}</span>
                  </div>
                  ...
                  {/* Excerpt */}
                  <p className="text-ae-muted text-xs leading-relaxed mb-4">{postLang.excerpt}</p>
